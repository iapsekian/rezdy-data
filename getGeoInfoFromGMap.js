...
let main = () => {
	MongoClient.connect(mdbUrl, (err, db) => {
		...
		var preparingData = () => {
			...
			//toursGeoCoded-XXX.json contains tours which has been geocoded. It was generated by updateTourTXByGeoInfo.js and read by this program for reducing the gmap API calling
			if(fs.existsSync('./mapping/toursGeoCoded-' + targetEnv +'.json')){
				toursGeoCoded = require('./mapping/toursGeoCoded-' + targetEnv +'.json'); 
			}

			var qualifyTours = () => {
				console.log('------------------ qualifyTours()');
		...
		var processingToursWithoutCoordinate = () => {
			console.log('------------ processingToursWithoutCoordinate()');
			...
			var wait4ToursWithoutCoordinateComplete = () =>{
				...
				if(0 === toursWithCityCountryCodeCount){
					...
					console.log(' *** Now Calling updateTourTXByGeoInfo.js as the next step ***');
