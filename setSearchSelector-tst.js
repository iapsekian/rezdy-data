		...
		.then((d) => {
			...
			d.forEach((item) => {
				...
				if(updateFlag || insertFlag){
					if(updateFlag){
						if(updateField.workspace.taxonomy[taxonomySSVocabularyId.searchSelector].length !== 0){
							updateField.workspace.taxonomy[taxonomySSVocabularyId.searchSelector].forEach((i)=>{
								tValue.push(i);
							});
						}
					}
