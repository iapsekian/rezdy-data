...
function step2GetProducts(){
	...
	function handleProductsResult(){
    	...
    	fs.writeFileSync('./logs/missingRecords-'+targetEnv+'.json', JSON.stringify(missingRecords));
...
function step3GetSuppliersByProducts(supplierAliasFromProducts){
	...
	function handleSupplierResult(){		
    	...
    	fs.writeFileSync('./datafiles/suppliers-'+targetEnv+'.json', JSON.stringify(jsonSuppliers));
	...
	MongoClient.connect(mdbUrl, (err, db) => {
		...
		existingProducts.forEach( (existingItem, existingIndex) => {
			...
			if(!putOnline){
				...
				if(ppu[1] !== 'tourcms'){ //add this if statement because if this Rezdy tour is selling on TourCMS, then we don't need to put it off-line
					if(ppu[0] !== 'https://bookur'){ //added this because of full payment to supplier tour. Record it down.
						payAttentionRToursLog += 'Put Off-line but productPageUrl does not equal to "bookur.rezdy.com", RTour - ' + existingItem.text + ' - Product Code = ' + existingItem.productCode + '\n';
					}
			...
			if(!putOnline){
				if(existingItem.marketplace === 'Rezdy'){
					...
					if(existingItem.productPageUrl.split('.')[0] !== 'https://bookur'){ //added because full payment to supplier tour has been dropped. they need to be recorded down.
						payAttentionToursLog += 'Put Off-line but productPageUrl does not equal to "bookur.rezdy.com", Tour - ' + existingItem.text + ' - Marketplace = ' + existingItem.marketplace + ' - Product Code = ' + existingItem.productCode + '\n';
					}
